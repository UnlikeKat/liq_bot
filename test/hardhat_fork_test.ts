...
async function testLiquidationOnFork(event: LiquidationEvent, testNumber: number): Promise<ForkTestResult> {
    ...
    try {
        console.log(`🔧 Creating Hardhat fork at block ${blockN3}...`);

        // Reset Hardhat Network to fork at specific block
        ...
        console.log(`✅ Fork ready at http://127.0.0.1:8545`);

        const forkClient = createPublicClient({ chain: base, transport: http('http://127.0.0.1:8545') });
        const account = privateKeyToAccount(CONFIG.PRIVATE_KEY as `0x${string}`);
        const forkWallet = createWalletClient({ account, chain: base, transport: http('http://127.0.0.1:8545') });
...
async function main() {
    ...
    console.log('⚙️  Using Hardhat Network (native Windows support)\n');
    ...
    for (let i = 0; i < testsToRun.length; i++) {
        ...
        writeFileSync('./test/results/hardhat_fork_results.txt', outputFile);
    }
    ...
    console.log(summary);
    writeFileSync('./test/results/hardhat_fork_results.txt', outputFile);
    console.log(`\n💾 Results: test/results/hardhat_fork_results.txt`);
}
